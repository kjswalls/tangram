...
import { CJK_PATTERN, hasCjk } from '@/lib/dict/rank';
 ...
 *
 * Deliberately below `RESOLVE_MAX_WORD_CHARS` (200), which is where
 * `DictStore.resolve` *rejects*: a pasted cell that ran long is a paste
 * problem and is truncated here, so it reaches the dictionary as a word that
 * will simply not match rather than as an error that stops the whole import.
 */
const MAX_WORD_CHARS = 100;

/**
 * A run of CJK ideographs, built from the repository's own `CJK_PATTERN`.
 *
 * `abe6793` spelled the ranges out a second time here, because at the time the
 * only copy lived in `lib/dict/search.ts` and that module dragged the
 * server-side JSON loader into whatever imported it. Neither half of that is
 * true any more: the pattern moved to `lib/dict/rank.ts`, which imports
 * nothing but types, and `data.md` D6 deleted the loader's route. So the
 * duplicate goes — `wave-zero.md` §10f is the entry about that pattern's
 * missing Extension G range, and it is worth exactly one place to fix.
 */
const CJK_RUN = new RegExp(`(?:${CJK_PATTERN.source})+`, 'u');

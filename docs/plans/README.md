...
- **The 63 MB on-device dictionary budget** propagates correctly from `data.md` D5a into `ios.md` I3 and
...
- **The licence boundary** — `decomp.json` never merged into the SQLite artifact, SQLCipher's BSD
  ...
  `data.md` D1/D5a, `core.md` C4, `ios.md` I3, `android.md` A5/A8 and `backend.md`'s `AskCacheRow` rule.
...
| **V2** | **SETTLED 2026-09-16 — the re-check ran and passed.** Every document that mentions the Practice-queue merge now assigns it: `core.md` C7 (its §2 scope list, §5 C7 and the file-disposition table), `ios.md` §4 ("Assigned by `wave-zero.md` §9 … so it is not assumed to be mobile work"), `wave-zero.md` §9 itself, and issue 10 above, which records the resolution under its statement of the problem. No third document repeats the "unassigned" claim. Original finding: ruling 9 landed on `core.md` and, until it was patched by hand, not on `ios.md`, whose scope table still called the Practice-queue merge unassigned — and `core.md` cited that stale row back as evidence the gap was real. Patched in both. Re-check that no third document repeats it. | nothing now |
| **V3** | **SETTLED 2026-09-16.** `android.md`'s ten `C5` citations are repointed: A6's floor argument now cites **C5a**, which is where `core.md` actually commits the degrade (`core.md:962` sits inside C5a, not C5b), its criteria 3 and 4 name C5a and C3–C5b, and A1's note says C7 sits after C5b. The one surviving bare `C5` narrates what an earlier draft said and is marked as the pre-split name. `ios.md` was already clean. The four live `D5` citations in `STACK.md` and this document are repointed to **D5a**, which is where the 63 MB budget is stated (`data.md:1049`). The two remaining in the settled-issues register above narrate the pre-split state and are left as history. Original finding: **phase ids that dangle after the two splits.** `D5` is cited in `STACK.md` (twice) and this document (twice); the 63 MB budget now lives in D5a. `C5` is cited ten times in `android.md` — A6's entire floor argument rests on "reading C5" and "C5's committed degrade", which are now C5a — and twice in `ios.md`. A6's acceptance criterion 3 names a spec id that no longer exists. | A6, and anyone reading the budget |
...
| **V6** | **SETTLED 2026-09-16 — executed and merged.** All ten modules are in `packages/ai/`, recorded by git as renames at 98–100% similarity, with **66** importers rewritten in the same commit — the session re-measured rather than trusting the count below, which six build tracks had made stale. `git grep "@/lib/ai/"` returns nothing. `apps/app/lib/ai/` is gone, correctly: §5 keeps `ask-client.ts` there and it does not exist yet (`backend.md` B2's remainder). V6's two other sub-points had already lapsed — `pnpm-workspace.yaml` landed in W0 and `package.json` is off the frozen list. This unblocks `backend.md` B1. Original finding: **wave 0's deliverable 5 names ten modules to move and zero importers.** Moving `lib/ai/**` to `packages/ai/` touches **33 files with 74 import sites** across the API routes, the lookup and review components, the dev seed, and sixteen test files. No document lists them or says they change in the same commit. Two further unstated points: the move lands before W0 creates `pnpm-workspace.yaml`, so `packages/` is briefly a bare directory reachable only through the `@/*` mapping; and `package.json` is on the current frozen list that deliverable 3 is meant to relax first. | wave 0 deliverable 5 |
| **V7** | **SETTLED 2026-09-16.** All nine repointed to section and phase ids: `backend.md`'s six (`core.md:191` ×2 → `core.md` §4's gate table; `data.md:674` ×3 → `data.md` §5 D3; `data.md:145` → §4's gate table, the D6 row) and `data.md`'s three (`core.md` line 184 ×2 → its §4 gate table; `backend.md:180` → §4's gate table). No cross-document line reference remains in any plan body. Original finding: stale line-number cross-references created by the parallel edits: `backend.md` cites `core.md:191` twice and `data.md:145`; `data.md` cites `core.md` line 184 twice and `backend.md:180`. All point at prose that moved. Prefer section and phase ids over line numbers. | nothing, but it misleads |
| **V8** | **SETTLED 2026-09-16.** Both were the same note, in `data.md` §3 and in D6: *"`core.md`'s gate table cites `data.md` §5.7"*. `core.md` no longer cites it — its only `5.7` is `STACK` §5.7, about Astro pages. Both rewritten to say the citation never resolved and that **D3** is the reference, keeping the dangling reference findable without asserting that anyone still makes it. Checked, and left alone, a third note that looked stale but is not: `data.md` §5 D1 quotes `core.md` C4 saying the dictionary-wide variant "is `data.md`'s call, not this plan's", and `core.md` still says exactly that. Original finding: two of `data.md`'s "the other document is wrong" notes are now themselves wrong, because the document they corrected has since been corrected. | nothing, but it misleads |
